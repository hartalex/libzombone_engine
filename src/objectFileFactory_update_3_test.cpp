...
int main(int argc, char *argv[]) {
  ...
  off.setup();
  std::cout << "Component should not yet be removed" << std::endl;
  c->isRemoved();
  c->remove();
  std::cout << "Component should be removed" << std::endl;
  c->isRemoved();
  std::cout << "Component should not be update" << std::endl;
  off.update();
  std::cout << "Component should still not be updated" << std::endl;
  return EXIT_SUCCESS;
}
